...
async def write_progress(resp: web.StreamResponse, progress: dict) -> bool:
    """Write one x_progress SSE frame. Returns False if the client is gone."""
...
def loading_progress(elapsed: float, instance: ModelInstance | None = None) -> dict:
    return {
...
async def keepalive_loop(resp: web.StreamResponse, instance: ModelInstance | None = None,
                         interval: float = 2.0, start: float | None = None):
    # The caller emits the first frame inline before creating this task.
    # asyncio.create_task only *schedules*: on a warm hit _load_model returns
    # without ever yielding to the event loop, so kl_task.cancel() fires before
    # this body runs a single line. A first frame emitted in here would never
    # reach the client. Poll-only by design - do not add an entry frame back.
    if start is None:
        start = time.monotonic()
    try:
        while True:
            await asyncio.sleep(interval)
            if not await write_progress(resp, loading_progress(time.monotonic() - start, instance)):
                return
    except (asyncio.CancelledError, ConnectionResetError, ConnectionAbortedError, BrokenPipeError):
        pass


def prompt_eval_progress(orchestrator, instance: ModelInstance, elapsed: float,
                         prompt_done: int, state: str, est_prompt_tokens: int) -> dict:
    progress = {
        "gen": 0, "think": 0,
...
async def _poll_slot(orchestrator, instance: ModelInstance) -> tuple[int, str]:
    """Returns (prompt_tokens_processed, phase). Falls back to 'prompt eval'."""
    state = "prompt eval"
    prompt_done = 0
    try:
...
async def prompt_eval_loop(orchestrator, resp: web.StreamResponse, instance: ModelInstance,
    ...
    # Poll-only: the caller emits the first "prompt eval" frame inline. See
    # keepalive_loop for why an entry frame here would be lost on a warm hit.
    if start is None:
        start = time.monotonic()
    try:
        while True:
            await asyncio.sleep(interval)
            prompt_done, state = await _poll_slot(orchestrator, instance)
            progress = prompt_eval_progress(
                orchestrator, instance, time.monotonic() - start,
                prompt_done, state, est_prompt_tokens,
            )
            if not await write_progress(resp, progress):
                return
    except (asyncio.CancelledError, ConnectionResetError, ConnectionAbortedError, BrokenPipeError):
        pass
        ...
        if is_stream:
            async with orchestrator._session.request(
                ...
                async for raw_chunk in upstream.content.iter_any():
                    ...
                    if cancelled:
                        break
                if upstream.status == 200 and not cancelled:
                    proc = instance.process
                    dead = proc is None or proc.poll() is not None
                    tail = (buf or b"").decode(errors="replace")
                    oom_kw = any(k in tail.lower() for k in ("out of memory", "out of video memory",
                                                             "cudamalloc", "cuda error", "oom"))
                    if dead or (oom_kw and not tail.startswith("data:")):
                        log.error("[%s] generation aborted by server failure for %s (dead=%s, tail=%s)",
                                  request.remote, instance.alias, dead, tail[:500])
                        frame = json.dumps({"error": {"message": f"Model '{instance.alias}' failed during "
                                                                  f"generation: {tail[:500] or 'server process died'}",
                                                      "type": "server_error"}})
                        try:
            ...
            async with orchestrator._session.request(
                ...
                instance.last_activity = time.monotonic()
                elapsed = time.monotonic() - proxy_start
                if upstream.status >= 500 or (
                        upstream.status in (200, 400) and upstream.content_type.startswith("text/plain")):
                    body_text = resp_body.decode(errors="replace")
                    if any(k in body_text.lower() for k in ("out of memory", "out of video memory",
                                                            "cudamalloc", "cuda error", "oom")):
                        log.error("[%s] OOM during generation for %s: %s",
                                  request.remote, instance.alias, body_text[:500])
                        return web.json_response(
                            {"error": {"message": f"Model '{alias}' ran out of VRAM during generation: {body_text[:500]}",
                                       "type": "out_of_memory"}},
                            status=502,
                        )
...
def setup_routes(app: web.Application, orchestrator):
    ...
    async def handle_proxy(request):
        ...
        if is_stream:
            ...
            try:
                ...
                if "closing transport" in str(e).lower() or "reset" in str(e).lower():
                    ...
                    return resp
                raise
            # Emit the first frame inline - create_task would not run before
            # _load_model returns on a warm hit. Only claim "loading" when the
            # model is genuinely not resident, otherwise a hot request would
            # show a load phase that never happens.
            req_start = time.monotonic()
            if not orchestrator._is_warm(alias):
                await write_progress(resp, loading_progress(0.0))
                kl_task = asyncio.create_task(keepalive_loop(resp, start=req_start))
        ...
        try:
            ...
            log.error("Model load failed for %s: %s", alias, reason)
            if resp is not None:
                err = json.dumps({"error": {"message": f"Failed to load model '{alias}': {reason}",
                                            "type": "server_error"}})
                try:
        ...
        if is_stream and resp is not None:
            ...
            # Same inline-first-frame rule as the loading phase: on a warm hit
            # the model can reach first token before a scheduled task ever runs.
            pe_start = time.monotonic()
            await write_progress(resp, prompt_eval_progress(
                orchestrator, instance, 0.0, 0, "prompt eval", est_tokens,
            ))
            pe_task = asyncio.create_task(
                prompt_eval_loop(orchestrator, resp, instance, est_tokens, start=pe_start)
            )
