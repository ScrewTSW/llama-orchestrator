...
class Orchestrator:
    ...
    def _calculate_vram_plan(self, model_path: Path, cfg: dict) -> dict:
        ...
        # RoPE scaling (YaRN) extends usable context beyond the trained window,
        # so the GGUF ceiling no longer applies. Only lift it when scaling is
        # explicitly configured - otherwise the clamp still protects against
        # asking for more than the model was trained on.
        if cfg.get("rope_scaling") and configured_ctx > max_model_ctx:
            log.info("Model %s: rope_scaling=%s, allowing configured ctx %d "
                     "beyond trained window %d",
                     model_path.name, cfg.get("rope_scaling"),
                     configured_ctx, max_model_ctx)
            max_model_ctx = configured_ctx
        ...
        if free_after_weights > 0:
            ...
            if configured_ctx > 0:
                # User-defined ctx is always respected, even if the KV cache
                # overflows into system RAM. Silent shrinking here is exactly
                # what broke hybrid models (40k instead of the configured value).
                ctx = min(configured_ctx, max_model_ctx)
                kv_mb = info.kv_cache_mb(ctx, ctk, ctv)
                if model_size_mb + kv_mb + compute_full_mb > available_mb:
                    log.warning("Model %s: configured ctx %d (KV %.0f MB) exceeds VRAM budget "
                                "(%.0f MB free) — KV will spill to system RAM",
                                model_path.name, ctx, kv_mb, free_after_weights)
            else:
    ...
    def _build_cmd(self, instance: ModelInstance) -> list[str]:
        ...
        if cfg.get("context_shift", True):
            cmd.extend(["--context-shift", "on"])
        if cfg.get("no_mmap", False):
            cmd.append("--no-mmap")
    ...
    async def _wait_for_health(self, instance: ModelInstance, timeout: float | None = None) -> bool:
        # Cold loads read the whole GGUF from disk; a large model on a cold page
    ...
    def _is_warm(self, alias: str) -> bool:
        """True if a request for `alias` will be served without a cold load.

        Used to decide whether to announce a loading phase at all - claiming
        one on a warm hit shows the client a phase that never happens.
        Deliberately lock-free: this is a display hint, and taking the lock
        here would yield to the event loop in the one path that must not.
        """
        inst = self.instances.get(alias)
        return bool(inst and (inst.alive or inst.loading))
            ...
            if not healthy:
                ...
                try:
                    ...
                    error_lines = [ln for ln in tail if any(
                        kw in ln.lower() for kw in ("failed", "error", "out of memory", "out of video memory", "cudamalloc", "cuda error", "cublas", "oom", "no space left", "abort")
                    )]
                    if error_lines:
